...
def detect(mask):    
    ...
    if isinstance(ls, int) : 
        ls = Line_System(7, np.pi / 4, (mask.shape[1] //2,  mask.shape[0] -  25 ), mask.shape  )
    
    distance, pt = ls.Get_Distances(mask, 2) 
    return ( {"distance" : distance},  pt.astype(np.uint8) ) #cv.cvtColor(Intersection, cv.COLOR_GRAY2RGB)  )
